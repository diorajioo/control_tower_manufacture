# Design Overview — Control Tower Manufacturing
...
This document is the global source of truth for design rules. Page-specific deviations are documented in `PAGES/`. When this document and a page doc conflict, the page doc wins — for that page only.

---

## Typography

**Lato exclusively.** Loaded from Google Fonts (weights 400 and 700 only).
...
### Specialty colors (fixed, never reassigned)
...
Default card style for dashboard pages:
...
| KPI padding | `px-[18px] py-4` |
| Chart padding | `p-3` to `p-4` |

See `PAGES/MONITOR.md` for Monitor-mode card overrides.
...
- Left: `"MANUFACTURING CONTROL TOWER"` — `text-[13px] font-medium text-white/50 uppercase tracking-[0.07em]`
- Right: clock · sync status · notification bell · Teams send button · avatar
- Row 2 (filter row): view toggle · period pills · date picker · plant dropdown · data level pills · monitor button
- View toggle only renders when `views` prop has 2+ items. Pass `views={[]}` to hide it.
...
### Charts (Nivo only)
**Do not switch to Recharts or any other chart library.**

Shared config in `lib/chartConfig.ts`:
- `fontFamily: "inherit"` — inherits Lato from body
- Axis tick text: `#9ca3af 10px`
- Grid lines: `#f3f4f6`
- Crosshair: `#215AA8`
- Tooltip bg: `#2A3D4A`
- SPC control zone fill: `#E9EFF6` at 55% opacity

TrendChart and StackedBarChart share a KPI selector — changing one updates the other (Tableau parity).
...
| Monitor button | `router.push('/monitor?page=...')` |
| Escape in fullscreen | `fullscreenchange` event → `router.back()` |
| 15min idle | Auto-logout → `/login` |

Animation principle: `transition-all ease-out` only. No `animate-bounce`, no spring physics.
...
- Monitor cards: inline `animate-pulse` divs matching card dimensions
...
| Don't | Do |
|---|---|
| Use Inter, Space Grotesk, or any non-Lato font | Use Lato (already global in `globals.css`) |
...
| Use `#215AA8` to signal KPI health | Use green/amber/red status colors |
| Use gradients anywhere in product UI | Only use gradient on AILabel chip |
...
| Use `animate-bounce` or spring animations | Use `transition-all ease-out` |
